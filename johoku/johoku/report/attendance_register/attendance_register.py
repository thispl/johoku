...
def get_data(filters):
    ...
    employees = sorted(
        employees,
        key=lambda emp: category_order.get(emp.employee_category, 99)
    )
...
def process_employee_data(emp, date_range, attendance_data, holiday_data_new, permission_data):
    ...
    for date in date_range:
        ...
        relieving=frappe.db.get_value('Employee',{'name':emp.name,'status':'Left'},['relieving_date'])
        if holiday and not holiday['holiday_date'] >= doj:
            holiday=''
        if holiday: 
            if relieving and not relieving > holiday['holiday_date'] :
                holiday=''
        if holiday:
            if holiday['weekly_off']:
                ...
                total_holiday += 1
        if att:
            if relieving and  relieving < date.date() :
                row1.append('-')
                row2.append('-')
                row3.append('-')
                row4.append('-')
...
def process_attendance_status(att, holiday, row1):
    ...
    leave = status_map.get(att['leave_type'], "")
    relieving=frappe.db.get_value('Employee',{'name':att['employee'],'status':'Left'},['relieving_date'])
    if relieving and  relieving < att['attendance_date'] :
        row1.append('-')
    else:

...
# Apply the outline border for the entire range
    # outline_range = ws.iter_rows(min_row=1, max_row=1, min_col=7, max_col=18)
    # for rows in outline_range:
    #     for cell in rows:
    #         cell.border = outline_border_new
    outline_range = ws.iter_rows(min_row=2, max_row=ws.max_row, min_col=1, max_col=18)
    for rows in outline_range:
        for cell in rows:
    ...
    for x in range(6,ws.max_row + 1):
        for rows in ws.iter_rows(min_row=x, max_row=x, min_col=1, max_col=18):
            for cell in rows:
                ...
                cell.border = border
    for x in range(6,ws.max_row + 1):
        for rows in ws.iter_rows(min_row=x, max_row=x, min_col=2, max_col=18):
            for cell in rows:
                cell.font = text_font_data

...
""", (from_date, to_date), as_dict=True)
    ...
    for s in salary_slips:
        ...
        from_date_str = datetime.strptime(str(from_date), "%d-%m-%Y")
        to_date_str = datetime.strptime(str(to_date), "%d-%m-%Y")

        present_days_count = 0
        halfdays_count = 0

        for single_date in frappe.utils.date_diff(from_date_str, to_date_str) + 1:
            current_date = from_date_str + timedelta(days=single_date)
            if not check_holiday(current_date, s['employee']):
            ...
            # Count the number of half days
                halfdays_count = frappe.db.get_value(
                    "Attendance", 
                    filters={
                        'employee': s['employee'],
                        'status': 'Half Day',  
        ...
        medical_allowance += salary_details['Medical Re imbursement']
    ...
    food_allowance,attire_allowance,mobile_allowance,medical_allowance,special_allowance,total_earnings_amount,total_el_amount,0,attendance_allowance,heat_allowance,
    travel_llowance,bonus,0,overtime,other_allowances,gross_pay,pf,esi,lwf,pt,0,others,loan,0,total_deduction,net_pay])
    return data

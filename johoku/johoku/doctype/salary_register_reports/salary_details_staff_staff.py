...
def make_xlsx(sheet_name=None):
    ...
    # ws.merge_cells(start_row=1, start_column=46, end_row=1, end_column=51)
    # ws.merge_cells(start_row=1, start_column=52, end_row=1, end_column=55)
    # ws.merge_cells(start_row=1, start_column=56, end_row=1, end_column=60)
    # ws.merge_cells(start_row=1, start_column=61, end_row=1, end_column=66)
    for row in data:
    ...
    # ws.merge_cells(start_row=2, start_column=46, end_row=3, end_column=51)
    # ws.merge_cells(start_row=2, start_column=52, end_row=3, end_column=55)
    # ws.merge_cells(start_row=2, start_column=56, end_row=3, end_column=60)
    # ws.merge_cells(start_row=2, start_column=61, end_row=3, end_column=66)
    ...
    # # ws.merge_cells(start_row=4, start_column=55, end_row=4, end_column=55)
    ...
    # ws.merge_cells(start_row=4, start_column=49, end_row=4, end_column=58)
    # ws.merge_cells(start_row=4, start_column=49, end_row=4, end_column=54)
    # ws.merge_cells(start_row=4, start_column=58, end_row=5, end_column=58)
    # ws.merge_cells(start_row=4, start_column=54, end_row=4, end_column=54)
    # ws.merge_cells(start_row=4, start_column=56, end_row=5, end_column=56)
    # ws.merge_cells(start_row=4, start_column=56, end_row=4, end_column=65)
    
    ws.merge_cells(start_row=1, start_column=12, end_row=1, end_column=46)
    # ws.merge_cells(start_row=1, start_column=41, end_row=1, end_column=44)
    ws.merge_cells(start_row=1, start_column=47, end_row=1, end_column=49)
    ...
    ws.merge_cells(start_row=2, start_column=1, end_row=2, end_column=46)
    # ws.merge_cells(start_row=2, start_column=41, end_row=3, end_column=44)
    ws.merge_cells(start_row=2, start_column=47, end_row=3, end_column=49)
    ...
    ws.merge_cells(start_row=3, start_column=1, end_row=3, end_column=46)
...
def apply_styles(ws):
    ...
    for rows in ws.iter_rows(min_row=1, max_row=2, min_col=46, max_col=66):
        for cell in rows:
    ...
    for rows in ws.iter_rows(min_row=4, max_row=5, min_col=26, max_col=35):
        for cell in rows:
            ...
            cell.alignment = align_center
            cell.border = border
    for rows in ws.iter_rows(min_row=4, max_row=5, min_col=56, max_col=56):
        for cell in rows:
            cell.font = text_font_header
            cell.alignment = align_center
            cell.border = border
    for rows in ws.iter_rows(min_row=4, max_row=5, min_col=66, max_col=66):
        for cell in rows:
            cell.font = text_font_header
            cell.alignment = align_center
            cell.border = border
    for rows in ws.iter_rows(min_row=4, max_row=4, min_col=17, max_col=26):
        for cell in rows:
            cell.font = text_font_header
            cell.alignment = align_center
            cell.border = border
    for rows in ws.iter_rows(min_row=4, max_row=4, min_col=37, max_col=46):
        for cell in rows:
            cell.font = text_font_header
            cell.alignment = align_center
            cell.border = border
    ...
    for rows in ws.iter_rows(min_row=4, max_row=4, min_col=46, max_col=48):
        for cell in rows:
            ...
            cell.alignment = align_center
            cell.border = border
    for rows in ws.iter_rows(min_row=5, max_row=5, min_col=49, max_col=55):
        for cell in rows:
            cell.font = text_font_header
            cell.alignment = align_center
            cell.border = border
    for rows in ws.iter_rows(min_row=4, max_row=4, min_col=57, max_col=63):
        for cell in rows:
            cell.font = text_font_header
            cell.alignment = align_center
            cell.border = border
    for rows in ws.iter_rows(min_row=4, max_row=4, min_col=54, max_col=54):
        for cell in rows:
            ...
            cell.alignment = align_center
            cell.border = border
    for rows in ws.iter_rows(min_row=4, max_row=4, min_col=55, max_col=55):
        for cell in rows:
            cell.font = text_font_header
            cell.alignment = align_center
            cell.border = border
    for rows in ws.iter_rows(min_row=4, max_row=5, min_col=56, max_col=56):
        for cell in rows:
            ...
            cell.alignment = align_center
            cell.border = border
    for rows in ws.iter_rows(min_row=4, max_row=5, min_col=36, max_col=36):
        for cell in rows:
            cell.font = text_font_header
            cell.alignment = align_center
            cell.border = border
    for rows in ws.iter_rows(min_row=4, max_row=4, min_col=49, max_col=54):
        for cell in rows:
            cell.font = text_font_header
            cell.alignment = align_center
            cell.border = border
    ...
    for x in range(6,ws.max_row + 1):
        for rows in ws.iter_rows(min_row=x, max_row=x, min_col=4, max_col=66):
            for cell in rows:
    ...
    for rows in ws.iter_rows(min_row=6, max_row=ws.max_row - 1, min_col=16, max_col=16):
        for cell in rows:
            cell.fill = PatternFill(fgColor="FABF8F", fill_type = "solid")
    # for x in range(26,34):
    for rows in ws.iter_rows(min_row=6, max_row=ws.max_row - 1, min_col=27, max_col=35):
        for cell in rows:
            cell.fill = PatternFill(fgColor="FABF8F", fill_type = "solid")
    for rows in ws.iter_rows(min_row=6, max_row=ws.max_row - 1, min_col=47, max_col=53):
        for cell in rows:
            cell.fill = PatternFill(fgColor="FABF8F", fill_type = "solid")
    for rows in ws.iter_rows(min_row=6, max_row=ws.max_row - 1, min_col=55, max_col=55):
        for cell in rows:
            cell.fill = PatternFill(fgColor="FABF8F", fill_type = "solid")
    for rows in ws.iter_rows(min_row=6, max_row=ws.max_row - 1, min_col=64, max_col=64):
        for cell in rows:
            cell.fill = PatternFill(fgColor="FABF8F", fill_type = "solid")
    for rows in ws.iter_rows(min_row=ws.max_row , max_row=ws.max_row , min_col=1, max_col=66):
        for cell in rows:
            cell.fill = PatternFill(fgColor="f4f087", fill_type = "solid")
            cell.border = border
...
def get_data(args):
    ...
    emp_basic = emp_house_rent_allowance = emp_conveyance_allowance = emp_education_allowance = emp_food_allowance = emp_attire_allowance = emp_special_allowance = 0
    ...
    total_deduction = net_pay = loan = others = pf = pt = lwf = esi = other_allowances = ot_hours = overtime = travel_llowance = gratuity = bonus = heat_allowance = tl_allowance = 0
    to_date_str =to_date
    total_tds =0
    ...
    data = [
            ...
            ["SALARY STATEMENT FOR THE MONTH OF " + month_year_format.upper() + " - STAFF"],
            ["S.No", "Emp Code", "Name", "DOB", "Grade", "DOJ", "Department", "Designation", "Aadhar No","PAN No", "UAN No.", "ESIC No.",
             "Bank A/C No", "Bank Type",  "Category", "No Working day", "Fixed Gross"] + 
            ...
            [""] * 10 + ["Basic", "HRA", "Conveyance", "Education", "Food Allow", "Attire Allowance", 
             ...
             "OT Amount", "Arear Allowance","", "EPF Round Off", "ESIC Round up off", "PT", "LWF", "Loan", 
              "Other Deduction",  "TDS Deduction"]
        ]
...
""", (from_date, to_date), as_dict=True)
    ...
    for s in salary_slips:
        ...
        emp_data = frappe.get_list(
            ...
            'mobile_allowance','medical_allowance','special_allowance','fixed_earning','esic_no','pan_no', "uan_no", "aadhar_no"
            ],
            limit_page_length=1
        )
        ...
        for doc_name in present_doc_name_list:
            ...
            if not check_holiday(attendance_date, s['employee']):
                ...
                if halfday_doc_name:
                    halfdays_count += 1
                # frappe.log_error(halfdays_count)
        ...
        components = ['Basic', 'House Rent Allowance', 'Conveyance', 'Education Allowance',
            ...
            'Attire Allowance', 'Mobile Allowance', 'Heat Allowance', 'Arear Allowance', 'Gratuity',
            'Overtime', 'Attendance Allowance','TL Allowance', 'Travel Allowance','Bonus', 'Provident Fund',
            'Employer State Insurance', 'Professional Tax', 'Loan', 'Others','Labour Welfare Fund','TDS Deduction']
        ...
        total_earnings = (
            salary_details['Basic'] + salary_details['House Rent Allowance'] + salary_details['Conveyance'] + salary_details['Education Allowance'] + 
            salary_details['Food Allowance'] + salary_details['Attire Allowance'] + salary_details['Mobile Allowance'] + 
            salary_details['Medical Re imbursement'] + salary_details['Special Allowance']
        )

        fromdate = datetime.strptime(str(s['date_of_birth']), '%Y-%m-%d')
        dob = fromdate.strftime('%d-%m-%Y')
        fromdate_ = datetime.strptime(str(s['date_of_joining']), '%Y-%m-%d')
        ...
        row = [
            ...
            emp.medical_allowance or 0, emp.special_allowance or 0, emp.fixed_earning or 0, s['total_present_days_of_employee'], holiday_data['total_holiday'],
            ...
            round(salary_details['Medical Re imbursement']), round(salary_details['Special Allowance']), round(total_earnings),
            round(salary_details['Attendance Allowance']), round(salary_details['Heat Allowance']), round(salary_details['TL Allowance']), round(el_amount), round(salary_details['Gratuity']), round(salary_details['Travel Allowance']), round(salary_details['Bonus']) , s['ot_hours'],
            round(salary_details['Overtime']), round(salary_details['Arear Allowance']), round(s['gross_pay']), round(salary_details['Provident Fund']),
            round(salary_details['Employer State Insurance'],0), round(salary_details['Professional Tax']), round(salary_details['Labour Welfare Fund']), round(salary_details['Loan']),
            round(salary_details['Others']),
            round(salary_details['TDS Deduction']), round(s['total_deduction']), round(s['net_pay'])
        ]
        data.append(row)
        ...
        present_days_count += s['total_present_days_of_employee']
        ...
        basic += round(salary_details['Basic'])
        hra += round(salary_details['House Rent Allowance'])
        conveyance += round(salary_details['Conveyance'])
        education_allowance += round(salary_details['Education Allowance'])
        food_allowance += round(salary_details['Food Allowance'])
        attire_allowance += round(salary_details['Attire Allowance'])
        mobile_allowance += round(salary_details['Mobile Allowance'])
        medical_allowance += round(salary_details['Medical Re imbursement'])
        special_allowance += round(salary_details['Special Allowance'])
        gross_pay += round(s['gross_pay'])
        total_el_amount += round(el_amount)
        total_earnings_amount += round(total_earnings)
        attendance_allowance += round(salary_details['Attendance Allowance'])
        heat_allowance += round(salary_details['Heat Allowance'])
        tl_allowance += round(salary_details['TL Allowance'])
        travel_llowance += round(salary_details['Travel Allowance'])
        gratuity += round(salary_details['Gratuity'])
        bonus += round(salary_details['Bonus'])
        ot_hours += s['ot_hours']
        overtime += round(salary_details['Overtime'])
        other_allowances += round(salary_details['Arear Allowance'])
        pf += round(salary_details['Provident Fund'])
        esi += round(salary_details['Employer State Insurance'],0)
        lwf += round(salary_details['Labour Welfare Fund'])
        pt += round(salary_details['Professional Tax'])
        others += round(salary_details['Others'])
        loan += round(salary_details['Loan'])
        total_deduction += round(s['total_deduction'])
        net_pay += round(s['net_pay'])
        total_tds +=round(salary_details['TDS Deduction'])
    data.append(["Total"]*15 +[total_working_days,emp_basic,emp_house_rent_allowance,emp_conveyance_allowance,emp_education_allowance,emp_food_allowance,emp_attire_allowance,
    ...
    food_allowance,attire_allowance,mobile_allowance,medical_allowance,special_allowance,total_earnings_amount,attendance_allowance,heat_allowance,
    tl_allowance,total_el_amount, gratuity, travel_llowance,bonus,ot_hours,overtime,other_allowances,gross_pay,pf,round(esi,0),pt, lwf, loan, others,total_tds,total_deduction,net_pay])
    return data

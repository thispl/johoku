...
class EmployeeCheckinWisePlanVsActualReport(Document):
    pass
    ...
    for x in range (10,ws.max_row+1):
        ...
        for rows in ws.iter_rows(min_row=x, max_row=x, min_col=2, max_col=24):
            for cell in rows:
                ...
                cell.alignment = align_right
                cell.border = border
        for rows in ws.iter_rows(min_row=x, max_row=x, min_col=29, max_col=31):
            for cell in rows:
                cell.font = text_font_data
                cell.alignment = align_right
                cell.border = border
        for rows in ws.iter_rows(min_row=x, max_row=x, min_col=32, max_col=32):
            for cell in rows:
                cell.font = text_font_data
                cell.alignment = align_left
                cell.border = border        
...
def get_data(args):
    if args.get('date'):
        ...
        for dept in departments:
            ...
            frappe.errprint(f"{dept} → Row length: {len(row)}")
            data.append(row)

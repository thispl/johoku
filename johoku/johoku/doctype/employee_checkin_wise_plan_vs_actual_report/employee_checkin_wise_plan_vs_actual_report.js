...
frappe.ui.form.on('Employee Checkin Wise Plan Vs Actual Report', {
    ...
    view: function(frm) {
        if (frm.doc.date) {  
            var path = "johoku.johoku.doctype.employee_checkin_wise_plan_vs_actual_report.employee_checkin_wise_plan_vs_actual_report.download";
            var args = $.param({
                date: frm.doc.date,
                shift: frm.doc.shift,
    ...
    get_data_system: function(frm) {
        ...
        if (frm.doc.date) {  
            frappe.call({
                ...
                method: "johoku.johoku.doctype.employee_checkin_wise_plan_vs_actual_report.employee_checkin_wise_plan_vs_actual_report.get_data_system",
                args: {
                    date: frm.doc.date,
                    shift: frm.doc.shift,
                    department: frm.doc.department

...
class TestEmployeeCheckinWisePlanVsActualReport(FrappeTestCase):
	pass
...
def get_data_system(date, shift): 
    ...
    for dept in departments:
        ...
        if dept.name != "All Departments":
            emp_category_name = ['Staff','Manager','ITI','TT','Naps','CL','GT','New CL','New Staff','New Trainees']
            ...
            planed_emp = frappe.db.get_all("Shift Assignment",{'department': dept.name,'start_date': date,'shift_type':shift1,'docstatus': 1,'employee_category': ['in',['Staff','Manager','ITI','TT','Naps','CL','GT','New CL','New Staff','New Trainees']]},pluck="employee")
            query = """
...
def apply_styles(ws):
    ...
    for rows in ws.iter_rows(min_row=1, max_row=9, min_col=1, max_col=32):
        for cell in rows:
            cell.fill = PatternFill(fgColor="ffe5b4", fill_type = "solid")
    for rows in ws.iter_rows(min_row=ws.max_row, max_row=ws.max_row, min_col=1, max_col=32):
        for cell in rows:
            cell.fill = PatternFill(fgColor="ffe5b4", fill_type = "solid")      
...
def get_data(args):
    if args.get('date'):
        ...
        data = [
            ...
            [""],
            ["", "Plan / Actual"] + [""] * 16 + ["Date: " + fromdate_]+ [""] * 9 + ["Shift: " + shift_header],
            [""],
            ...
            for emp_category in employee_categories:
                ...
                if emp_category_name == "Staff":
                    ...
                    actual_cl_query = f"""
                        ...
                        ORDER BY time
                    """
                    result = frappe.db.sql(actual_cl_query, (date, dept.name, emp_category_name), as_dict=True)
                    actual_cl = len(result)

...
def make_xlsx(sheet_name=None):
    ...
    ws["L1"].alignment = Alignment(horizontal='left', vertical='top', wrap_text=True)
    ...
    ws.merge_cells(start_row=1, start_column=12, end_row=1, end_column=44)
...
def apply_styles(ws):
    ...
    for rows in ws.iter_rows(min_row=1, max_row=2, min_col=44, max_col=63):
        for cell in rows:
    ...
    for rows in ws.iter_rows(min_row=6, max_row=ws.max_row - 1, min_col=26, max_col=33):
        for cell in rows:
            cell.fill = PatternFill(fgColor="ed8c7b", fill_type = "solid")
    for rows in ws.iter_rows(min_row=6, max_row=ws.max_row - 1, min_col=45, max_col=50):
        for cell in rows:
            cell.fill = PatternFill(fgColor="ed8c7b", fill_type = "solid")
    for rows in ws.iter_rows(min_row=6, max_row=ws.max_row - 1, min_col=52, max_col=52):
        for cell in rows:
            cell.fill = PatternFill(fgColor="ed8c7b", fill_type = "solid")
    for rows in ws.iter_rows(min_row=6, max_row=ws.max_row - 1, min_col=60, max_col=60):
        for cell in rows:
            cell.fill = PatternFill(fgColor="ed8c7b", fill_type = "solid")
    for rows in ws.iter_rows(min_row=ws.max_row , max_row=ws.max_row , min_col=1, max_col=63):
        for cell in rows:
            cell.fill = PatternFill(fgColor="ede577", fill_type = "solid")
...
# Convert to datetime object if it's a string
    ...
    data = [
            # [""] * 2 + ["JOHOKU MANUFACTURING PRIVATE LIMITED"] + [""] * 32 + ["Prepared By"] + [""] * 4 + 

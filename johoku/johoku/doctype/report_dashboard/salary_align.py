...
def make_xlsx(sheet_name=None):
    ...
    ws["AB1"].value = "Prepared By"
    ws["AF1"].value = "Checked By"
    ws["AJ1"].value = "Verified By"
    ws["AO1"].value = "Approved By"
    ...
    ws.merge_cells(start_row=1, start_column=42, end_row=1, end_column=45)
    for row in data:
    ...
    ws.merge_cells(start_row=2, start_column=42, end_row=3, end_column=45)
    ...
    for x in range(17,27):
        ws.merge_cells(start_row=4, start_column=x, end_row=5, end_column=x)
    ws.merge_cells(start_row=4, start_column=36, end_row=5, end_column=36)
    ...
    ws.merge_cells(start_row=4, start_column=30, end_row=4, end_column=35)
...
def apply_styles(ws):
    ...
    for x in range(2,4):
        for rows in ws.iter_rows(min_row=x, max_row=x, min_col=1, max_col=45):
            for cell in rows:
                cell.font = header_font
                cell.alignment = align_center
                cell.border = border
    for rows in ws.iter_rows(min_row=4, max_row=5, min_col=1, max_col=45):
        for cell in rows:
            cell.font = text_font_header
            cell.alignment = align_center
            cell.border = border
    for rows in ws.iter_rows(min_row=1, max_row=1, min_col=28, max_col=45):
        for cell in rows:
            cell.font = header_font
            cell.alignment = align_center
            cell.border = border
    for x in range(6,ws.max_row + 1):
        for rows in ws.iter_rows(min_row=x, max_row=x, min_col=4, max_col=45):
            for cell in rows:
    ...
    for rows in ws.iter_rows(min_row=6, max_row=ws.max_row - 1, min_col=17, max_col=24):
        for cell in rows:
            cell.fill = PatternFill(fgColor="FABF8F", fill_type = "solid")
    for rows in ws.iter_rows(min_row=6, max_row=ws.max_row - 1, min_col=30, max_col=32):
        for cell in rows:
            cell.fill = PatternFill(fgColor="FABF8F", fill_type = "solid")
    for rows in ws.iter_rows(min_row=6, max_row=ws.max_row - 1, min_col=34, max_col=35):
        for cell in rows:
            cell.fill = PatternFill(fgColor="FABF8F", fill_type = "solid")
    for rows in ws.iter_rows(min_row=6, max_row=ws.max_row - 1, min_col=41, max_col=43):
        for cell in rows:
            cell.fill = PatternFill(fgColor="FABF8F", fill_type = "solid")
    for rows in ws.iter_rows(min_row=ws.max_row , max_row=ws.max_row , min_col=1, max_col=45):
        for cell in rows:
            cell.fill = PatternFill(fgColor="f4f087", fill_type = "solid")
            cell.border = border
...
def get_data(args):
    ...
    data = [
            [""],
            ["SALARY STATEMENT FOR THE MONTH OF " + month_year_format + " - Graduate Trainee"],
            ...
            [""] * 2 + ["Present Days", "S/PH", "COFF", "CL", "SL/ML", "EL", "N/F", "LOP", "Absent", "NPD", 
             "GROSS EARNINGS"] + [""] * 2 + ["VARIABLE EARNINGS"] + [""] * 5 + 
            ["Total Earning", "Deductions"] + [""] * 7 + ["Net Pay"],

            [""] * 13 + 
            ["Basic", "HRA","Fixed Gross"] + 
            [""] * 10 + ["Basic", "HRA", "Total Gross Earning", 
            "Attendance Allowance", "Heat Allowance", "OT Hrs", 
            "Overtime Amount", "Arrear Allowance", "EL Encashment","", "EPF Round Off", "ESIC", "Prof Tax", 
...
""", (from_date, to_date), as_dict=True)
    ...
    for s in salary_slips:
        ...
        present_days_count_ = frappe.db.count(
            "Attendance", 
            filters={
                'employee': s['employee'],
                'status': 'Present',  
        ...
        components = ['Basic', 'House Rent Allowance', 'Conveyance', 'Education Allowance',
            'Food Allowance', 'Medical Reimbursement', 'Special Allowance','Arrear Allowance',
        ...
        if s['employee'] == emp.name:
            ...
            row = [
                ...
                present_days, holiday_data['total_holiday'],
                coff_count, cl_count, sl_count, el_count, holiday_data['total_other_holiday'], lop_count,s['absent_days'],s['payment_days'],
                salary_details['Basic'],salary_details['House Rent Allowance'],s['gross_pay'],
                salary_details['Attendance Allowance'], salary_details['Heat Allowance'],0,
                salary_details['Overtime'],salary_details['Arrear Allowance'],el_amount, total_earnings, salary_details['Provident Fund'],
                salary_details['Employer State Insurance'],salary_details['Professional Tax'],0,0,salary_details['Labour Welfare Fund'],
                salary_details['Others'],s['total_deduction'], s['net_pay']
            ]
            data.append(row)
            ...
            total_arrear_allowance += salary_details['Arrear Allowance']
    ...
    gross_pay,attendance_allowance,heat_allowance,
    0,overtime,total_arrear_allowance,total_el_amount,total_earnings_amount,pf,esi,pt,0,lic,lwf,others,total_deduction,net_pay])
    return data
